...
public class IntroductionManager : MonoBehaviour
{
    ...
    (string, bool, bool, bool, bool, bool)[] instructions = new (string, bool, bool, bool, bool, bool)[]
    {
        ("To continue through scenes, look at the next button", false, false, false, false, false),
        ("Try to familiarize yourself with the keyboard layout", false, false, false, false, false),
        ("We will now begin teaching letter typing", false, false, false, false, false),
        ("AND", true, true, false, false, false),
        ("AREA", true, true, false, false, false),
        ("HOLE", true, true, false, false, false),
        ("BRAVE", true, true, false, false, false),
        ("SHIRT", true, true, false, false, false),
        ("COMB", true, true, false, false, false),
        ("DRAFT", true, true, false, false, false),
        ("BANANA", true, true, false, false, false),
        ("CRISIS", true, true, false, false, false),
        ("LIMB", true, true, false, false, false),
        ("ELEVEN", true, true, false, false, false),
        ("HAND", true, true, false, false, false),
        ("GARAGE", true, true, false, false, false),
        ("KNOCK", true, true, false, false, false),
        ("OTHER", true, true, false, false, false),
        ("RELAX", true, true, false, false, false),
        ("WORK", true, true, false, false, false),
        ("WINDOW", true, true, false, false, false),
        ("PURPLE", true, true, false, false, false),
        ("MOMENT", true, true, false, false, false),
        ("SHOWER", true, true, false, false, false),
        ("DRIVING", true, true, false, false, false),
        ("COMPUTER", true, true, false, false, false),
        ("ORGANIZE", true, true, false, false, false),
        ("BEDFRAME", true, true, false, false, false),
        ("Now, we'll actually give you the opporunity to try and type. You'll be typing the same words, so make sure you know the path. Look ahead now and trace the path you'll need to type. When ready, go next", false, false, false, false, false),
        ("The system is ready for you to type. To mark the beginning and end of your words, use the spacebar on the keyboard. Press space to begin, draw out the word, and press space again, and it should interpret your writing.", false, false, false, false, false),
        ("BOND", false, true, false, false, false),
        ("APPLE", false, true, false, false, false),
        ("FRIEND", false, true, false, false, false),
        ("TABLE", false, true, false, false, false),
        ("PLAYERS", false, true, false, false, false),
        ("SECURITY", false, true, false, false, false),
        ("Now you will be given the ability to type full sentences, and use the ring here as if it were a spacebar", false, false, true, false, false),
        ("A LITTLE TALKING IS NEEDED", true, true, true, false, false),
        ("FALL IS MY FAVORITE SEASON", true, true, true, false, false),
        ("PLAYERS MUST KNOW ALL THE RULES", true, true, true, false, false),
        ("FEBRUARY HAS AN EXTRA DAY", true, true, true, false, false),
        ("BAD DRIVING RESULTS IN A FINE", true, true, true, false, false),
        ("NOW, YOU WILL BE GIVEN THE OPPORTUNITY TO TRY TYPING WITH YOUR EYES ONLY. WHEN READY, PRESS NEXT AND BEGIN.", false, false, false, false, false),
        ("A LEAF DROPS TO THE GROUND", true, true, true, false, false),
        ("THE NIGHT SKY IS BRIGHT AND CLEAR", true, false, true, true, false),
        ("A SMALL STREAM FLOWS UNDER THE BRIDGE", true, true, false, false, true),
        ("GENTLE WAVES LAP AT THE SHORE", false, true, true, true, false),
        ("A LIGHT BREEZE MOVES THE TABLE", true, false, true, false, true),
    };
    ...
    private float howGreen = 1f;
    private float longwait = .9f;
    private float lastWord = 2.0f;
        ...
        while (true)
        {
            ...
            foreach (char letter in word)
            {
                if (InSameGroup(letter.ToString(), lastLetter))
                {
                    ...
                    if (firstPass && letterNum != 0)
                    {
                        DrawLineBetweenPoints(previousLetter, center);
                    }
                ...
                foreach (KeyValuePair<string, GameObject> pair in letterGroups)
                {
                    ...
                    else if (letter.ToString() == " ")
                    {
                        ...
                        yield return new WaitUntil(() => ring.LookingAtBox(EyePos.gazeLocation, EyePos.worldPosition) || Time.time - startTime > timeoutDuration);
                        ...
                        if (Time.time - startTime > timeoutDuration)
                        {
                            ...
                            centerBox.turnGreen(howGreen);
                            yield return new WaitUntil(() =>
                                ring.LookingAtBox(EyePos.gazeLocation, EyePos.worldPosition));
                        }
            ...
            keyboard.setInput(word);

            yield return wordPause;
        }
    }

    IEnumerator DrawLineAsGazeMoves(GameObject startEntity, GameObject endEntity)
    {
        if (currentLine == null)
        {
            currentLine = Instantiate(linePrefab);
        ...
        LineRenderer lineRenderer = currentLine.GetComponent<LineRenderer>();
        lineRenderer.positionCount = 2;
        Vector3 startPos = startEntity.transform.position;
        Vector3 endPos = endEntity.transform.position;

        while (true)
        {
            Vector3 gazePos = EyePos.gazeLocation;
            Vector3 clampedGazePos = Vector3.Lerp(startPos, endPos, Vector3.Dot((gazePos - startPos).normalized, (endPos - startPos).normalized));

            lineRenderer.SetPosition(0, startPos);
            lineRenderer.SetPosition(1, clampedGazePos);

            if (Vector3.Distance(clampedGazePos, endPos) < 0.1f)
            {
                break;
            }
    ...
    public bool InSameGroup(string currLetter, string lastLetter)
    {
        ...
        if (lastGrouping.Contains(currLetter))
        {
            return true;
        }
        else
