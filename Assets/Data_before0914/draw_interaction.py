...
file_pre = './Tutorial/2/'
file_suf = '/interaction_record.txt'
...
for i in range(1, iterations+1):
    ...
    for j in range(len(lines)):
        line = lines[j].rstrip()
        parts = line.split(', ')
        if parts[2] != iter and parts[2] != '0':
            if not true_flag:
                if prev_parts[1] == "True":
                    time_guide.append(5.0)
                else:
                    time_noguide.append(5.0)
            iter = parts[2]
            prev_parts = None
            true_flag = False
        if parts[4] == "True" and not true_flag:
            ...
            if parts[1] == "True":
                ...
                success_noguide += 1
        prev_parts = parts
    # print(time_guide, time_noguide, success_guide, success_noguide)
...
plt.savefig('interaction_2.png', dpi=300)
